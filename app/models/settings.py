...
class Settings:
    ...
    def update_form_value(self, field: str, value: Any, project_id) -> None:
        ...
        if len(value) == 0 or not value:
            query = Query()
            query.Delete("setting_fields").Where("project_id = " + str(int(project_id))).Where(
            ...
            query = Query()
            query.Select("project_id").From("setting_fields").Where("field = " + self.dbo.q(field)).Where(
                "project_id = " + self.dbo.q(project_id)
            )
            self.dbo.execute(query)
            result = self.dbo.fetch_one()
            if result:
                self.dbo.update_object(
                    "setting_fields", field_value.model_dump(db_fields=True), ["field", "project_id"], True
                )
            else:
